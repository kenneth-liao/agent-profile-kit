...
launching from a descendant prevents Profile Context from loading. Skills-only
Codex Profiles generate no hook and require no hook approval.
...
It reports current, not installed, stale source, missing output, drifted output, blocked and
malformed ownership, while keeping Host configuration warnings visible.
