#!/usr/bin/env python3
"""Terminal evidence capture for #653 / TEST-006 (method of the 0.217.1 capture).
...
ROOT = pathlib.Path("/private/tmp/apkit-653-session")
CANDIDATE = pathlib.Path("/private/tmp/apkit-653-candidate")
...
IMAGE_FRAME_LABELS = (
    ...
    "05-first-install",
    "07-status",
    "11-selection-host-picker",
    "11-selection-filtered-host",
    "15-details-list",
    "17-host-missing-warning",
    ...
    try:
        for pattern, answer, frame in steps:
            ...
            try:
                ...
                pass
            if frame:
                snapshot(screen, f"{label}-{frame}", width, command)
            p.send(answer)
...
def inspect_picker_redraw() -> dict:
    ...
    for stream_id, raw in raw_streams.items():
        if "selection" not in stream_id and "11-" not in stream_id:
            continue
...
def main() -> None:
    ...
    for width in (100, 60):
        ...
        run(
            ...
            "02-setup",
            ["init", "./workspace"],
            [(r"\?[^\r\n]*\(y/N\)", "y\r", "confirmation")],
        )
        context = workspace / "context" / "team.md"
        context.parent.mkdir(parents=True, exist_ok=True)
        context.write_text("Use clear, concise language.\n")
        run(width, "03-create-profile", ["new", "profile", "engineering", "--context", "team"], capture=False)
        ...
        run(
            width,
            "05-first-install",
            ["install", "engineering", str(hello), "--host", "codex", "--auto-confirm"],
        )
        ...
        run(
            width,
            "09-failure",
            ["install", "enginering", str(hello), "--host", "codex", "--auto-confirm"],
            expected=1,
        )
        run(
            width,
            "10-invalid-target",
            ["install", "engineering", str(base / "missing"), "--host", "codex", "--auto-confirm"],
            expected=1,
        )

        choice = base / "projects" / "selection"
        choice.mkdir(parents=True, exist_ok=True)
        # Interactive install: detected-Host preselection, filtered multi-select,
        # confirmation review, then a plain decline (neutral cancellation).
        # The settled filter line is `›` + ANSI + ` cod…`; match the raw bytes
        # after the reset, not the plain `› cod` transcript (DEC-12).
        run(
            width,
            "11-selection",
            ["install"],
            [
                (r"Which Profile", "\r", "profile-picker"),
                (r"Which Agent Hosts", "cod", "host-picker"),
                (r"\x1b\[0m cod", "\x1b[B\r", "filtered-host"),
                (r"Install into this Project", "n\r", "install-confirmation"),
            ],
            expected=1,
            cwd=choice,
        )
        ...
        for n in ("alpha/my-app", "beta/my-app", "acme-internal-analytics-pipeline-v2"):
            ...
            run(
                width,
                "seed-" + n.replace("/", "-"),
                ["install", "engineering", str(target), "--host", "codex", "--host", "claude", "--auto-confirm"],
                capture=False,
            )
        run(width, "12-fleet-inventory", ["list", "projects"])
        run(width, "13-fleet-status", ["status", "--all"])
        context.write_text("Use clear, concise language. Explain each result briefly.\n")
        ...
        run(
            ...
            [
                (r"Which Profile", "\r", "profile-picker"),
                (r"Which Agent Hosts", "\r", "host-picker"),
                (r"Install into this Project", "y\r", "confirmation"),
            ],
            cwd=home_proj,
        )

        # Missing-Host warning naming Projects (US-011): remove stubs while
        # the fleet is still installed (keeps label 17 for frame compare).
        clear_host_stubs(base)
        run(width, "17-host-missing-warning", ["update"])

        # Partial outcome: one Project commits, one fails under a read-only
        # root, then details shows Written/Failed/Pending (US-008).
