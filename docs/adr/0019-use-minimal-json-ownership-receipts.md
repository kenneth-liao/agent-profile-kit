...
Spec #371 (tickets #374, #379, #376) removed the ownership-evidence machinery this record describes. The Installation Marker no longer exists: the Installation Receipt in Installation State is the sole canonical ownership home, so identity proof through a Project-travelling token is gone. Repository Exclusion Contributions are derived, best-effort cache rewrites that can never block or require repair, and receipts no longer record an exclusion target or entry list. Agent Host capability probing is advisory and can no longer gate writes. See ADR-0025 for the accepted reduction.
