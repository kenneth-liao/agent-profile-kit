...
bindings. `status` and `apply` select the complete fleet by default, accept `--here`
...
Reconciliation returns global Blockers plus one complete record per Project, ordered by canonical Project identity. A Project record owns desired identity, state, observable output operations and their consuming Hosts, Project Blockers, typed structured warnings and copyable values, Host Setup Steps, and Git exclusion changes. No report consumer joins parallel Project or path collections. Blockers carry typed facts only — kind, scope, Project identity, affected items, and per-kind non-prose evidence such as occupation type, ownership action, and diagnostic detail; the Installer emits no user-facing sentence. Presentation (`cli/blocker-wording.ts`) is the single home of every problem, requirement, and remedy sentence, keyed by the typed `BlockerKind`: sentences are authored once in plain newcomer language with atomic command parts, the machine JSON publishes the flattened projection verbatim, and human rendering displays the same authored parts with commands whole and copyable. The lifecycle selection flags select the same Projects for display, machine reports, and apply writes: `--stale` selects existing installations needing updates or restoration (changed source, changed or missing generated files) and excludes never-installed, removal-pending, and Blocked Projects; `--blocked` selects Projects with Project-scoped Blockers. The filters are mutually exclusive, compose with fleet or explicit Project scope, never narrow inspection (classification and global safety checks still run over the full scope), and never filter away Global Blockers; under a filter, apply writes only the selected membership, orphaned-receipt retirement is suppressed, and every selected committed write stays visible in the Apply Receipt (DEC-006, DEC-040). For Git-tracked output ownership conflicts, every view's remedy carries the exact `--literal-pathspecs` untracking command, staging index-only removal while the working files stay on disk, derived from the Blocker's recorded paths. Human lifecycle rendering consumes these nested typed records directly, authors task language without semantic regex translation, and renders each Blocker's complete structured evidence. The apply receipt remains the pre-apply work record, while a fresh post-commit reconciliation remains authoritative for resulting state.
