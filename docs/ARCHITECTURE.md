...
- `list` prints the available read-only inventory topics and examples. `list projects` reads the normalized Local Configuration model and renders every Project Binding's short Project identity, Profile, and normalized Host set; `--json` emits the same records with a canonical Project path when resolvable and the authored path retained for stale roots in a versioned payload. It does not inspect Git, project output, Installation State, or Host capabilities, and it does not hide current Projects when lifecycle status is clean.
