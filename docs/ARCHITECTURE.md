...
Initialization writes no example material: a new Workspace contains only `workspace.yaml` and the three artifact directories (spec #593 DEC-003, ADR-0047); example Profiles and Context Modules live in the guides and `apkit guide` examples, authored through the single authoring-examples authority.
