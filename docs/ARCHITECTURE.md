...
home-relative Project root, and use `--all` as their only fleet scope. `preview`
remains fleet-only. `bind` and `unbind` are recording-only; hand-editing Local Configuration remains valid,
