...
eventual `update --all`. Successful `bind` and `unbind` receipts are compact
...
existing absolute or home-relative Project root, and retain `--all` as explicit fleet scope. `bind` and `unbind` are recording-only; hand-editing Local Configuration remains valid,
