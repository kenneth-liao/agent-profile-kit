...
- `list` prints the available read-only inventory topics and descriptions from one canonical topic table. Focused human inventory presents instructional usage rather than an optional or redundant `Next:` action; JSON remains unchanged.
  ...
  - `list hosts` reads normalized Host inventory records from capability constants and renders the canonical Hosts supported for configured Projects in its concise human view, with guidance to select a listed Host through `install`. The human view labels each Host's executable detection as detected/not found — the one shared wording with the install Host picker (#669) — using the advisory Host detection authority (`detectInstalledHosts()`), never implying Profile loading; detection decides by executable presence on `PATH` alone and never starts a Host or writes (spec #593, US-009, DEC-012), so an executable absent from `PATH` — or an exhausted lookup budget — degrades to "not found" and every Host stays selectable. The `PATH` presence check is the recorded targeted exception to the no-process inventory boundary (ADR-0016 amendment for #512, narrowed by its #594 amendment): detection performs no process execution and writes no state, and the topic's `--json` view remains the byte-stable, probe-free capability-metadata envelope (DEC-009).
