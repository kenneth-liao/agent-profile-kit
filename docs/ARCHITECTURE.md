...
Agent Host capability probing is advisory (ADR-0025): the Adapter owns each Host's version floor and project-surface requirements, and probing classifies capability for warning purposes only. Deduplication is one advisory warning per identical Host capability failure per invocation, keyed by the Host and its failure message: a requirement that fails identically for every Project — a Host CLI missing from `PATH`, for example — emits once per invocation regardless of how many Projects select the Host, while distinct requirement sets for the same Host and Project-specific surface failures keep their distinct warnings. Capability evidence never gates planning, never gates writing, and never produces a Blocker — the planned material is written regardless, and the Adapter never silently omits or weakens what it plans. `status` performs no Host process execution; probing happens only during `apply` and temporary installation, and its failure is never fatal. Host authentication, project trust, and approval flows remain native concerns; Agent Profile Kit never writes global trust or authentication state.
