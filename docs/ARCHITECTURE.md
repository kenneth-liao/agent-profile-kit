...
Every bound root, Git or non-Git, uses the same removal semantics. The ordinary order is `unbind`, `update`, then delete the directory. If the directory is deleted first, the binding remains desired state until `unbind` matches its exact authored path. That explicit removal confirms intent and retires the Project's active Installation Receipt in the same operation; generated files remain until the next `update`, which proves and removes them as it does for any retired receipt. Restoring the project later requires a new `install`, which starts a clean receipt lifetime without manual state editing. Worktrees receive no special deletion or recovery mechanism.
...
eventual `update --all`. Successful `install` and `unbind` receipts are compact
...
bindings. `status` and `update` select the complete fleet by default, accept `--here`
for the bound Project containing the current working directory, accept one explicit
existing absolute or home-relative Project root, and retain `--all` as explicit fleet scope. `install` records and installs in one action while `unbind` only removes the binding; hand-editing Local Configuration remains valid.
...
teardown authority whose recorded detail the next `update` consumes to prove and
