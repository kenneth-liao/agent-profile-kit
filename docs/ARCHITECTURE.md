...
Skills conform to the portable subset of the Agent Skills standard. Standard Skill package members are projected with source bytes and modes preserved, except for two projection-owned edits: the generated-source notice that opens every projected `SKILL.md` body after frontmatter (ADR-0041), and Host-native translation of optional model-invocation policy (`metadata.agent-profile-kit.model-invocation`, default `allowed`). Agents, Hooks, and Tools are not Profile selection categories and are not delivered: no Adapter projects them and no Host section consumes them.
...
(ADR-0041), and omitting the `agent-profile-kit.yaml` sidecar. Codex and Pi use
...
(ADR-0041), and the Agent Profile Kit sidecar is omitted. Native Antigravity
