...
Presentation may order, filter, group, and simplify these records but cannot derive Host knowledge or setup completion. Concise `status` renders no Host Setup Steps. Concise `update` renders standing trust or root-launch guidance only when its Apply Receipt creates the first relevant output for that Project/Host pairing, and renders transition-triggered approval only when the associated output is added, updated, or repaired. It groups guidance by user action and Host without exposing provenance headings or separate consequence lines. Shared-path explanations and complete provenance, consequences, and Project scopes remain verbose and JSON evidence. Blocked `update` suppresses guidance for work that did not happen; a no-op `update` shows no setup and does not claim next-launch Activation. Agent Profile Kit does not claim that Host-owned trust or approval is complete.
