...
- `list` prints the available read-only inventory topics and examples from one canonical topic table. `list projects` reads the normalized Local Configuration model and renders every Project Binding's short Project identity, Profile, and normalized Host set; a per-binding path problem is retained instead of aborting the inventory. `--json` emits the same records with engine provenance, a canonical Project path when resolvable, and the authored path retained for recovery. It does not inspect Workspace artifacts, Git, project output, Installation State, or Host capabilities, and it does not hide current Projects when lifecycle status is clean.
