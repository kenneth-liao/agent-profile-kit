...
Skills conform to the portable subset of the Agent Skills standard. Standard Skill package members are projected with source bytes and modes preserved, except for two projection-owned edits: the generated-source notice that opens every projected `SKILL.md` body after frontmatter (ADR-0041) and Host-native translation of optional model-invocation policy (`metadata.agent-profile-kit.model-invocation`, default `allowed`). A Skill package containing the retired `agent-profile-kit.yaml` sidecar fails Workspace validation; the sidecar never reaches planning or projection. Context Module frontmatter `dependencies` keys are tolerated but never read (ADR-0045). Agents, Hooks, and Tools are not Profile selection categories and are not delivered: no Adapter projects them and no Host section consumes them.
