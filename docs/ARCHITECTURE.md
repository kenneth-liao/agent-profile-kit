...
- `install` records one Project Binding in Local Configuration and installs its verified output in the same action: the requested Profile and Host set become the exact desired final set for the Project (an existing installation is replaced, reported as an old → new receipt), then the generated output is planned, written, and verified for that Project. Selection publication serializes with other lifecycle writers through the Local Configuration lock with a snapshot recheck and atomic replacement; output installation serializes through the installation lifecycle lock, which re-ingests the configuration and fails closed on drift. A pre-commit failure restores the previous selection where possible; a post-commit verification failure keeps the committed selection and reports truthfully.
