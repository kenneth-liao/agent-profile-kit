...
The user's single canonical source of Profiles, Context, Skills, Agents, Hooks, and Tools consumed by Agent Profile Kit. It may own both Profile-selected artifacts and unselected universal artifacts; selection controls managed project delivery, not whether material is valid Workspace source. The fixed default path is `~/.agents/agent-profile-kit/workspace/`; Local Configuration may optionally select one existing absolute or home-relative Workspace path on this machine instead. It may be a Git repository version-controlled independently of the tool.
...
**Local Configuration**:
The untracked source of machine-specific, non-secret values, including Project Bindings and an optional Workspace path, that bind canonical definitions to the current system. Omitting `workspace` retains the fixed default Workspace path.
