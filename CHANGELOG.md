...
- Give the occupied-Skill-destination diagnostic a structured runnable `apkit new skill` recovery command after its explanation (DEC-014, US-022, INT-1 review finding on [#473](https://github.com/kenneth-liao/agent-profile-kit/pull/473)), and make Skill-creation failure recovery safe against user-content deletion: the exclusive open is the sole ownership boundary, so a pre-open failure never removes any file, only a successful exclusive open authorizes removing the leaf file on later failure, cleanup preserves every entry it cannot prove invocation-created, and the typed `skill-creation-residue` fact classifies surviving material as entirely Agent Profile Kit-created or containing foreign content with presentation-owned complete recovery guidance for each (PROD-1 review finding on [#473](https://github.com/kenneth-liao/agent-profile-kit/pull/473)).
