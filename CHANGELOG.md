...
- Add a focused human `status --blockers-only` view: every global and Project Blocker in the selected scope with concise deterministic grouping and no unrelated lifecycle inventory; `--blockers-only --verbose` retains every affected item and complete Blocker field without unrelated sections, `--blockers-only --json` fails before lifecycle inspection with focused usage guidance, and a Blocker-free scope reports that outcome without ordinary inventory ([#351](https://github.com/kenneth-liao/agent-profile-kit/issues/351)).
