...
- Make `uninstall` proceed per Project instead of failing globally. Each Project is evaluated and removed independently: a Project whose owned output cannot be fully removed is reported as kept with its reason and skipped, every other Project is still fully removed in the same invocation, and a skipped Project alone never returns a tool error. Only removed Projects' receipts are retired from Installation State, so a kept Project stays installed with its exclusion entries intact. The result reports what was removed and what was kept, per Project ([#381](https://github.com/kenneth-liao/agent-profile-kit/issues/381)).
