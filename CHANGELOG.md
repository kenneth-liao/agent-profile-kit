...
- Tell the user how to verify Host loading after apply: a successful apply that committed installation work follows its readiness statement with one concrete Project-local action — start a new session of each configured Agent Host in the updated Project and confirm that the installed material is in effect — naming the applied Profiles, the configured Hosts, and the updated Project from apkit-owned facts; no-op, blocked, and failed applies and machine JSON omit it, the sentence never claims Agent Profile Kit observed the loading or completed Host-owned setup, and Host-specific loading knowledge stays Adapter-owned (US-041, DEC-025, OOS-009, TEST-001, TEST-002, TEST-015, [#457](https://github.com/kenneth-liao/agent-profile-kit/issues/457)).
- End the first run with an authoring handoff: an apply whose committed receipt shows it installed the scaffolded example Profile — output additions for a Project bound to the example, derived purely from apply/example evidence with no persisted onboarding state — closes its human report with a concrete handoff to authoring real material using the delivered `apkit new` commands (`apkit new skill`, `apkit new context`, and `apkit new profile` with explicit selections, each an atomic copyable command), while routine applies — refreshed outputs, no-op applies, applies of user-authored Profiles — and machine JSON omit the teaching (US-040, DEC-024, TEST-001, TEST-002, TEST-015, [#456](https://github.com/kenneth-liao/agent-profile-kit/issues/456)).
- Guide the first Profile during initialization: on an interactive input stream, `init` may offer optional first-Profile guidance when the resolved target Workspace has no Profiles and selectable material exists — a yes/no offer (default no), a Profile name, and multi-select choices of the target's existing Context Modules and Skills (a category with no material is skipped) — collecting every decision before initialization commits any change, so cancellation at any prompt exits with a cancellation diagnostic and no configuration change or generated output; a completed flow initializes through the unchanged initialization operation, creates the Profile through the delivered `apkit new profile` scaffolding path (shared implementation, zero selections and invalid or occupied names refused before any write with the typed facts), and prints the equivalent fully specified `apkit new profile` command so re-running it needs no answers again; non-interactive invocations, re-initialization with Profiles present, and ambiguous targets never prompt and keep the delivered initialization behavior (US-054, US-045 interactive clauses, init clauses of US-052/US-055–056, DEC-030–035, TEST-001, TEST-002, TEST-017, TEST-019, [#460](https://github.com/kenneth-liao/agent-profile-kit/issues/460)).
- Extend the prompt seam with yes/no (default no) and free-text question kinds on the same injectable-streams/clock carriage contract, and re-reference the injected input stream at each question: releasing a finished question unreferences real TTY input so the CLI can exit, and a later question of the same flow re-references it, so sequential prompts keep working on a terminal (first exercised by guided init's multi-question flow) (US-054, DEC-035, [#460](https://github.com/kenneth-liao/agent-profile-kit/issues/460)).
- Prompt for missing binding arguments: interactive `bind` asks only for the missing required Profile and Host arguments — the Profile choice is preceded by a one-sentence explanation of what a Profile is (wording relocated to one shared home with the initialization receipt), the Host choices carry advisory installed/absent detection evidence for every supported Agent Host, and a completed flow records the binding through the unchanged binding operation and prints the equivalent fully specified command with the Project path and every `--host` flag explicit; a fully specified bind bypasses prompts entirely, cancellation exits before any configuration change with a cancellation diagnostic (exit 1), non-interactive invocations and empty/unreadable Profile inventories keep the delivered missing-argument errors, and the prompt seam gains select and multiselect kinds on the same injectable-streams/clock carriage contract (US-051, US-053, US-033 selection-flow clause, bind clauses of US-052/US-055–056, DEC-030–033, DEC-035, TEST-001, TEST-016, TEST-019, [#459](https://github.com/kenneth-liao/agent-profile-kit/issues/459)).
 - Create Profiles through the CLI with explicit material selections: `apkit new profile <name> [--context <id>]... [--skill <id>]...` scaffolds one valid bindable Profile into the configured Workspace at `profiles/<name>.yaml`, resolving every selected name through the existing Workspace boundary — unknown selections are refused with the delivered missing-reference facts carrying sorted available names and the nearest-name suggestion, and zero selections are refused with the typed `profile-without-artifacts` fact, now carrying the same available-names guidance — while the receipt names the selected material, prints available Context Module and Skill names as guidance, and prints the actual full file path; the command never prompts or opens an editor on any input stream (US-044, US-045 non-interactive clauses, US-046 Profile clause, US-055 new-Profile non-interaction clause, DEC-026, TEST-001, TEST-002, TEST-017, TEST-019, [#454](https://github.com/kenneth-liao/agent-profile-kit/issues/454)).
- Create Context Modules through the CLI: `apkit new context <name>` scaffolds one valid Context Module into the configured Workspace at `context/<name>.md` and prints the actual full file path, never prompting or opening an editor on any input stream; the scaffold is preflighted through the canonical Context Module schema before any write, occupied or symlinked destinations are refused without being written through, failed creations never leave unreported partial material, and the shared `skill-path-occupied`/`skill-creation-residue` typed facts are generalized to `artifact-path-occupied`/`artifact-creation-residue` with unchanged Skill behavior and wording (US-043, US-046 Context Module clause, US-055 new-Context-Module non-interaction clause, DEC-026, TEST-001, TEST-002, TEST-017, TEST-019, [#453](https://github.com/kenneth-liao/agent-profile-kit/issues/453)).

- Make bare `apkit` invocation a read-only entry screen showing current setup state instead of the full manual: an uninitialized machine is told setup is missing and given `apkit init` (with a legacy-configuration variant), and a configured machine sees its fleet summarized by primary-cause counts plus the settled count from the delivered default fleet scope, followed by a short task-relevant command list (`status`, `apply`, `bind`, `guide`) with the canonical summaries; machine-facing commands are structurally omitted and the full surface stays reachable only through an `apkit --help` pointer, while the screen writes nothing on the machine (US-032, US-035 bare-invocation clause, DEC-020, DEC-021, TEST-001, TEST-002, TEST-015, TEST-020, [#452](https://github.com/kenneth-liao/agent-profile-kit/issues/452)).

- Confirm changed-output replacement before writes: interactive `apply` names every affected changed generated file with its Project attribution and asks before any invocation write — even for fully specified scope arguments — with the default answer no; declining, cancelling, or the default aborts the whole invocation before any configuration or generated-output write, including other healthy selected Projects and pending retirement work; acceptance or the explicit `--replace-changed` flag permits replacement without bypassing ownership, path-safety, or Global Blockers, while non-interactive and `--json` invocations never prompt and retain complete replacement receipts; a completed confirmation prints the equivalent fully specified command with `--replace-changed` so re-running it needs no second answer (US-029-031, US-062, apply clauses of US-052/US-055-056, DEC-019, DEC-030-033, DEC-035-036, TEST-001, TEST-002, TEST-014, TEST-019, TEST-022, [#458](https://github.com/kenneth-liao/agent-profile-kit/issues/458)).

- Select the same Projects for reports and writes: `--stale` selects existing installations needing source updates or generated-file restoration (changed source, changed or missing generated files) and excludes never-installed, removal-pending, and Blocked Projects; `--blocked` selects Projects with Project-scoped Blockers. Both flags are mutually exclusive (DEC-038), compose with fleet, `--here`, and explicit Project scope, and govern human reports, machine JSON (`schemaVersion: 15`), and apply write scope identically: filtered apply changes no unselected Project, suppresses orphaned-receipt retirement, and keeps every selected committed write visible in the Apply Receipt, while Global Blockers stay visible and effective and exit-code meanings are unchanged. The former `--blockers-only` flag is removed with guidance to `--blocked`, and the former prohibition on combining it with `--json` is gone (US-011, US-012 filter-composition clause, US-059-061, DEC-006, DEC-038, DEC-040, TEST-001, TEST-002, TEST-007, TEST-021, [#455](https://github.com/kenneth-liao/agent-profile-kit/issues/455)).
...
- Keep the receipt-proven Workspace source-change cause visible in verbose diagnostics when a normalized dependency or inclusion-reason change alters the desired-input digest while every generated projection stays byte-identical and concurrent output drift masks the source state: reconciliation records the Project input-change fact independently of primary state, one predicate drives both the stale-source classification and the fact, and verbose renders the cause once at Project scope only when no changed projection truthfully owns it (review INT-1 and RE-1 on [#477](https://github.com/kenneth-liao/agent-profile-kit/pull/477), [#449](https://github.com/kenneth-liao/agent-profile-kit/issues/449)).

### Changed

- Make verbose lifecycle output focused diagnostics omitting composed Context bodies, unchanged output paths, Capability Contract identifiers, and per-Project setup provenance, while preserving multi-cause project diagnostic evidence — per-output drift causes, blockers, state explanations, and the receipt-proven Workspace source-change cause rendered beside its affected paths or Project state — and actionable host setup steps (US-013, DEC-006, DEC-007, TEST-001, TEST-002, TEST-008, [#449](https://github.com/kenneth-liao/agent-profile-kit/issues/449)).
