...
- Keep the receipt-proven Workspace source-change cause visible in verbose diagnostics when a normalized dependency or inclusion-reason change alters the desired-input digest while every generated projection stays byte-identical and concurrent output drift masks the source state: reconciliation records the Project input-change fact independently of primary state, and verbose renders it once at Project scope only when no changed projection truthfully owns it (review INT-1 on [#477](https://github.com/kenneth-liao/agent-profile-kit/pull/477), [#449](https://github.com/kenneth-liao/agent-profile-kit/issues/449)).
