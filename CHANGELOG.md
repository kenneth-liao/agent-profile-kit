...
- Give the occupied-Skill-destination diagnostic a structured runnable `apkit new skill` recovery command after its explanation (DEC-014, US-022, INT-1 review finding on [#473](https://github.com/kenneth-liao/agent-profile-kit/pull/473)), and recover partial Skill-creation writes by cleaning up only material proven created by the failing invocation, reporting typed residual-path evidence with presentation-owned retry guidance when cleanup fails while preserving pre-existing and foreign material (PROD-1 review finding on [#473](https://github.com/kenneth-liao/agent-profile-kit/pull/473)).
