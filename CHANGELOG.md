...
- Group concise tracked-path Blocker evidence by deterministic immediate parent-directory prefix with lossless counts, and print the exact user-owned `git rm --cached --` untracking command (safely quoted, canonically ordered, never executed) in focused verbose `status` and `apply` views with recovery copy that preserves working files and retains the Project Binding alternative; ordinary concise and verbose views point to the focused diagnostics instead ([#353](https://github.com/kenneth-liao/agent-profile-kit/issues/353)).
