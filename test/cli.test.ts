...
describe("agent-profile-kit project-bound lifecycle", () => {
  ...
  test("validate directs a Workspace with no configured Projects to install one", async () => {
    const home = isolatedHome();
    await initialize(home);
  ...
  test("validate reports a nested Profile file with its path and fix (#598)", async () => {
    ...
    // Duplicate Profile IDs are structurally impossible — one file per name —
    // so the old duplicate-id diagnostic can no longer occur through
    // ingestion; the Profile file-name rule's violations take its place.
    mkdirSync(join(profiles, "archive"), { recursive: true });
    writeFileSync(join(profiles, "archive", "old-work.yaml"), "context: [example-context]\nskills: []\n");
  ...
  test("missing bootstrap files do not affect validate, status, update, or uninstall", async () => {
    const home = isolatedHome();
    const workspace = workspacePath(home);
    mkdirSync(workspace, { recursive: true });
    writeFileSync(join(workspace, "workspace.yaml"), "schema_version: 1\n");
  ...
  test("symlinked configured Workspace aliases keep installation identity across update and status", async () => {
    ...
    const applyViaLink = await runCli(home, "update");
  ...
  test("validate normalizes home-relative project roots and does not invoke Codex", async () => {
    ...
    const bin = join(home, "bin");
    // A controlled Codex stub is already staged under home/bin; overwrite it
  ...
  test("update and status default to fleet scope, including from an unbound working directory", async () => {
    const home = isolatedHome();
    await initialize(home);
    removeScaffoldedExample(home);
    writeContextProfile(home);
    ...
    const apply = await runCliAt(home, unbound, "update");
    expectExitCode(apply, 0);
    expect(existsSync(join(first, ".agent-profile-kit"))).toBe(true);
    expect(existsSync(join(second, ".agent-profile-kit"))).toBe(true);
  });

  test("--here explicitly scopes status and update to the bound Project containing the working directory", async () => {
    const home = isolatedHome();
    await initialize(home);
    removeScaffoldedExample(home);
    writeContextProfile(home);
    ...
    const apply = await runCliAt(home, join(selected, "."), "update", "--here");
    expectExitCode(apply, 0);
    expect(existsSync(join(selected, ".agent-profile-kit"))).toBe(true);
    expect(existsSync(join(unrelated, ".agent-profile-kit"))).toBe(false);
  });

  test("blocked status and update preserve scope in recovery retry guidance and never offer fleet widening on scoped runs", async () => {
    const home = isolatedHome();
    await initialize(home);
    removeScaffoldedExample(home);
    writeContextProfile(home);
    ...
    // 2. Blocked update --here from inside secondProject
    const applyHere = await runCliAt(home, secondProject, "update", "--here");
    expectExitCode(applyHere, 2);
    expect(humanText(applyHere.stdout)).toContain("Resolve the reported blocker, then run apkit update --here again.");
    expect(applyHere.stdout).not.toContain("then run apkit update again.");
    ...
    const globalBlockedApplyHere = await runCliAt(home, firstProject, "update", "--here");
    expectExitCode(globalBlockedApplyHere, 2);
    expect(humanText(globalBlockedApplyHere.stdout)).toContain("Resolve the reported global blocker, then run apkit update --here again.");
  ...
  test("--here in an unbound working directory fails with actionable guidance", async () => {
    ...
    for (const command of ["update", "status"] as const) {
      ...
      expect(failed.stderr).toContain("Run apkit install");
      expect(failed.stderr).toContain("Run apkit list projects");
      expect(failed.stderr).toContain(`Usage: apkit ${command}`);
    }
  });

  test("status and update accept one explicit absolute or home-relative bound Project root", async () => {
    const home = isolatedHome();
    await initialize(home);
    removeScaffoldedExample(home);
    writeContextProfile(home);
    ...
    expectExitCode(
      await runCli(home, "update", "~/projects/home-relative-project"),
      0,
    );
  });
  ...
  test("scoped lifecycle does not inspect or mutate unrelated Project output or Git state", async () => {
    ...
    // probes the unrelated binding would fail before updating the selected Project.
    writeFileSync(
      configPath(home),
      `schema_version: 2\nworkspace: ${workspacePath(home)}\nbindings:\n` +
        `  - project: ${selected}\n    profile: coding\n    hosts: [codex]\n` +
  ...
  test("scoped lifecycle rejects unbound, ambiguous, missing, relative, wildcard, and invalid targets before writes", async () => {
    ...
    const ambiguousJson = await runCliAt(home, nested, "update", "--here", "--json");
  ...
  test("status reports desired additions without writing project, state, or host configuration", async () => {
    ...
    expectExitCode(result, 0);
    expect(humanText(result.stdout)).toContain(humanText(`${projectPath}: not installed yet`));
  ...
  test("status leads with a concise ready-to-update outcome and grouped change counts", async () => {
    ...
    expect(result.stdout).toContain("Next: apkit update");
  ...
  test("update and status preserve Codex configuration warnings without blocking installation", async () => {
    ...
    const apply = await runCli(home, "update", "--verbose");
    expectExitCode(apply, 0);
    ...
    expectExitCode(status, 0);
    expect(humanText(status.stdout)).toContain(humanText(`${projectPath}: up to date`));
  ...
  test("blocked update renders one update report without duplicate stderr blockers", async () => {
    ...
    expectExitCode(result, 2);
    expect(result.stdout.startsWith("✖ Update blocked\n")).toBe(true);
  ...
  test("status and update accept --verbose and --json while rejecting other presentation arguments", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = project();
    writeContextProfile(home);
  ...
  test("status spawns no Agent Host process and update writes material when the Host CLI is outdated", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = project();
    writeContextProfile(home);
    ...
    const apply = await runCliWithPath(home, oldCodexBin, "update", "--all", "--json");
    expectExitCode(apply, 0);
    ...
    const noopApply = await runCliWithPath(home, goodCodexBin, "update", "--all", "--json");
  ...
  test("a missing Host emits one warning per invocation while update writes every Project", async () => {
    ...
    const apply = await runCliWithPath(home, emptyBin, "update", "--all", "--json");
    expectExitCode(apply, 0);
  ...
  test("an outdated Host with distinct requirement sets emits one warning per Host per invocation", async () => {
    ...
    const apply = await runCliWithPath(home, oldClaudeBin, "update", "--all", "--json");
    expectExitCode(apply, 0);
  ...
  test("status and update share a uniform exit-code matrix for clean, blocked, and tool-error states", async () => {
    const cleanHome = isolatedHome();
    await initialize(cleanHome);
    const cleanProject = project();
    writeContextProfile(cleanHome);
    bind(cleanHome, cleanProject);
    expectExitCode(await runCli(cleanHome, "update"), 0);
  ...
  test("update creates the manifest, composed Context, and native SessionStart hook", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = project();
    writeFileSync(join(projectPath, "AGENTS.md"), "repository-owned\n");
  ...
  test("successful update reports verified current state and a separate apply receipt", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = project();
    writeContextProfile(home);
  ...
  test("later Skill addition on an established Codex pairing does not replay standing first-use", async () => {
    ...
    writeContextProfile(home);
    bind(home, projectPath);
    const first = await runCli(home, "update");
  ...
  test("verbose update labels pending and updated work separately", async () => {
    ...
    const applied = result.stdout.indexOf("Updated:");
  ...
  test("update leaves current installation outputs and state untouched", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = project();
    writeContextProfile(home);
  ...
  test("tracked destinations block even when the tracked file is currently absent", async () => {
    ...
    expect(apply.stdout).toContain("Update blocked");
    expect(humanText(apply.stdout)).toContain(
      humanText("Agent Profile Kit must exclusively manage its generated files"),
    );
  });

  test("status --stale selects only stale Projects in a mixed fleet, agreeing with JSON (#455)", async () => {
    const home = isolatedHome();
    await initialize(home);
    removeScaffoldedExample(home);
    writeContextProfile(home);
    const projectStale = project("agent-profile-kit-stale-drifted-");
    const projectSettled = project("agent-profile-kit-stale-settled-");
    const projectNever = project("agent-profile-kit-stale-never-");
    writeFileSync(
      configPath(home),
      `schema_version: 2\nworkspace: ${workspacePath(home)}\nbindings:\n` +
        `  - project: ${projectStale}\n    profile: coding\n    hosts: [codex]\n` +
        `  - project: ${projectSettled}\n    profile: coding\n    hosts: [codex]\n`,
    );
    expectExitCode(await runCli(home, "update", "--all"), 0);
    // The never-installed Project joins after the applied fleet, so its
    // Profile Installation has never existed.
    writeFileSync(
      configPath(home),
      `schema_version: 2\nworkspace: ${workspacePath(home)}\nbindings:\n` +
        `  - project: ${projectStale}\n    profile: coding\n    hosts: [codex]\n` +
        `  - project: ${projectSettled}\n    profile: coding\n    hosts: [codex]\n` +
        `  - project: ${projectNever}\n    profile: coding\n    hosts: [codex]\n`,
    );
    // Changed generated bytes make the settled installation stale.
    writeFileSync(join(projectStale, ".codex", "hooks.json"), "hand edit\n");
    ...
    const machine = await runCli(home, "status", "--all", "--stale", "--json");
    ...
    expect(payload.projects.map((entry) => entry.canonicalProject)).toEqual([
      realpathSync(projectStale),
    ]);
  });

  test("status --stale counts missing generated files as stale while --blocked does not (#455, TEST-007)", async () => {
    const home = isolatedHome();
    await initialize(home);
    removeScaffoldedExample(home);
    writeContextProfile(home);
    const projectPath = project("agent-profile-kit-missing-output-");
    bind(home, projectPath);
    expectExitCode(await runCli(home, "update"), 0);
    // A wholly absent owned output under an existing installation is missing,
    // not never-installed: restoration is stale work.
    rmSync(join(projectPath, ".codex", "hooks.json"));
    ...
    expectExitCode(blocked, 0);
    expect(humanText(blocked.stdout)).not.toContain(humanText(projectPath));
  });

  test("the selection filters compose with explicit Project scope and --here (US-012)", async () => {
    const home = isolatedHome();
    await initialize(home);
    removeScaffoldedExample(home);
    writeContextProfile(home);
    const stale = homeGitRepository(home, "compose-stale");
    const settled = homeGitRepository(home, "compose-settled");
    writeFileSync(
      configPath(home),
      `schema_version: 2\nworkspace: ${workspacePath(home)}\nbindings:\n` +
        `  - project: ${stale}\n    profile: coding\n    hosts: [codex]\n` +
    ...
    expectExitCode(await runCli(home, "update", "--all"), 0);
    writeFileSync(join(stale, ".codex", "hooks.json"), "hand edit\n");
    ...
    expectExitCode(hereStale, 0);
    // The containing Project renders under the shared current-directory alias.
    expect(humanText(hereStale.stdout)).toContain("generated files changed");
    expect(humanText(hereStale.stdout)).not.toContain("~/projects/compose-settled");
    ...
    expect(
      readFileSync(join(settled, ".agent-profile-kit", "codex", "context.md"), "utf8"),
    ).not.toContain("Scoped composition change.");
  });

  test("filtered reports retain global Blockers and their exit code (#455, TEST-021)", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = project();
    writeContextProfile(home);
    ...
    expectExitCode(stale, 2);
    expect(stale.stdout).toContain("Global blockers:");
    expect(stale.stdout).toContain("installation record");

    const machine = await runCli(home, "status", "--stale", "--json");

    expectExitCode(machine, 2);
    const payload = JSON.parse(machine.stdout) as {
      readonly outcome: string;
      readonly globalBlockers: readonly unknown[];
    };
    expect(payload.outcome).toBe("blocked");
    expect(payload.globalBlockers).toHaveLength(1);
  });

  test("status --blocked selects only Project-scoped Blockers and --stale excludes them (#455)", async () => {
    const home = isolatedHome();
    await initialize(home);
    removeScaffoldedExample(home);
    writeContextProfile(home);
    const projectStale = project("agent-profile-kit-blocked-stale-");
    const projectBlocked = project("agent-profile-kit-blocked-blocked-");
    execFileSync("git", ["init", "-q", projectBlocked]);
    mkdirSync(join(projectBlocked, ".codex"), { recursive: true });
    writeFileSync(join(projectBlocked, ".codex", "hooks.json"), "tracked placeholder\n");
    execFileSync("git", ["-C", projectBlocked, "add", ".codex/hooks.json"]);
    rmSync(join(projectBlocked, ".codex", "hooks.json"));
    writeFileSync(
      configPath(home),
      `schema_version: 2\nworkspace: ${workspacePath(home)}\nbindings:\n` +
        `  - project: ${projectStale}\n    profile: coding\n    hosts: [codex]\n` +
        `  - project: ${projectBlocked}\n    profile: coding\n    hosts: [codex]\n`,
    );
    // Install only the stale candidate; the Git-tracked Project stays never-installed
    // and blocked by its ownership evidence, so --stale must exclude it either way.
    expectExitCode(await runCli(home, "update", "--all"), 2);
    writeFileSync(join(projectStale, ".codex", "hooks.json"), "hand edit\n");

    const blockedHuman = await runCli(home, "status", "--all", "--blocked");

    expectExitCode(blockedHuman, 2);
    expect(humanText(blockedHuman.stdout)).toContain(humanText(projectBlocked));
    expect(humanText(blockedHuman.stdout)).not.toContain(humanText(projectStale));
    ...
    const payload = JSON.parse(blockedMachine.stdout) as {
      readonly projects: readonly { readonly canonicalProject: string }[];
    };
    expect(payload.projects.map((entry) => entry.canonicalProject)).toEqual([
      realpathSync(projectBlocked),
    ]);
  });

  test("--stale and --blocked are mutually exclusive with actionable guidance (US-059)", async () => {
    const home = isolatedHome();

    for (const command of ["status", "update"] as const) {
      const result = await runCli(home, command, "--stale", "--blocked");
  ...
  test("the removed --blockers-only flag points at --blocked (#455)", async () => {
    const home = isolatedHome();

    for (const command of ["status", "update"] as const) {
      const result = await runCli(home, command, "--blockers-only");

      expectExitCode(result, 1);
      expect(result.stderr).toContain("--blockers-only");
      expect(result.stderr).toContain("--blocked");
    }
  });

  test("status --blocked selects Project-scoped Blockers in every invocation mode (#455)", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = join(home, "tracked-project");
    mkdirSync(projectPath);
    ...
    const blocked = await runCli(home, "status", "--all", "--blocked");

    expectExitCode(blocked, 2);
    expect(blocked.stdout).toContain("is tracked by Git, so Agent Profile Kit cannot");
    expect(blocked.stdout).toContain("Resolve the reported blocker");

    const blockedHere = await runCliAt(home, projectPath, "status", "--blocked");

    expectExitCode(blockedHere, 2);
    expect(blockedHere.stdout).toContain("is tracked by Git, so Agent Profile Kit cannot");

    // The selected membership is identical on the machine surface (US-061).
    const blockedJson = await runCli(home, "status", "--all", "--blocked", "--json");
    expectExitCode(blockedJson, 2);
    const payload = JSON.parse(blockedJson.stdout) as {
      readonly schemaVersion: number;
      readonly outcome: string;
    ...
    // A globally blocked apply under the filter performs no writes and keeps
    // the blocked exit code with the blocked report (write-scope proof).
    const blockedApply = await runCli(home, "update", "--all", "--blocked");
    expectExitCode(blockedApply, 2);
    expect(blockedApply.stderr).toBe("");
    expect(blockedApply.stdout).toStartWith("✖ Update blocked");
    expect(blockedApply.stdout).toContain("is tracked by Git, so Agent Profile Kit cannot");
    expect(existsSync(join(projectPath, ".codex", "hooks.json"))).toBe(false);

    const blockedApplyVerbose = await runCli(home, "update", "--all", "--blocked", "--verbose");
    expectExitCode(blockedApplyVerbose, 2);
    expect(blockedApplyVerbose.stdout).toContain("is tracked by Git, so Agent Profile Kit cannot");
    expect(blockedApplyVerbose.stdout).toContain("Affected path:");
    expect(existsSync(join(projectPath, ".codex", "hooks.json"))).toBe(false);

    const blockedVerbose = await runCli(home, "status", "--all", "--blocked", "--verbose");

    expectExitCode(blockedVerbose, 2);
    expect(blockedVerbose.stdout).toContain("is tracked by Git, so Agent Profile Kit cannot");
    expect(blockedVerbose.stdout).toContain("Affected path:");
    expect(blockedVerbose.stdout).toContain(".codex/hooks.json");
  ...
  test("status --blocked with no Blocked Projects renders an empty selection (#455)", async () => {
    const home = isolatedHome();
    await initialize(home);
    removeScaffoldedExample(home);
  ...
  test("packed regression groups many tracked generated paths into one explained blocker with zero writes", async () => {
    ...
    expect(status.stdout).toContain("Cannot update");
    ...
    expect(status.stdout).not.toContain("--blockers-only");
    ...
    const focusedVerbose = await runCli(home, "status", "--all", "--blocked", "--verbose");
  ...
  test("a Git binding reconciles only its exact root and preserves local exclusions", async () => {
    ...
    expect(humanText(currentStatus.stdout)).toContain(humanText(`${repository}: up to date`));
  ...
  test("scoped update changes only its contribution to a shared Git exclusion target", async () => {
    const home = isolatedHome();
    await initialize(home);
    ...
    const apply = await runCliWithPath(home, pathWithClaude, "update", nested, "--json");
    expectExitCode(apply, 0);
  ...
  test("shared Git exclusions use one canonical record and retain surviving contributions", async () => {
    ...
    expect(installed.schema_version).toBe(9);
    expect(installed.receipts).toHaveLength(2);
    for (const receipt of installed.receipts) {
      expect(receipt.repository_exclusion).toBeUndefined();
    }
    const section = readFileSync(target, "utf8");
    for (const entry of [
      "/.agent-profile-kit/codex/context.md",
      "/.codex/hooks.json",
  ...
  test("retires a deleted Git root and removes only its recorded exclusion contribution", async () => {
    ...
    const status = await runCli(home, "status", "--verbose");
    expectExitCode(status, 0);
    expect(humanText(status.stdout)).toContain(humanText(`${nested}: needs attention (removal: project intentionally deleted)`));
  ...
  test("retires one deleted independent project while preserving another installation", async () => {
    ...
    const status = await runCli(home, "status", "--verbose");
    expectExitCode(status, 0);
    expect(humanText(status.stdout)).toContain(humanText(`${first}: needs attention (removal: project intentionally deleted)`));
    expectExitCode(await runCli(home, "update"), 0);
  ...
  test("a missing shared exclusion section during intentional-deletion retirement is republished by update", async () => {
    const home = isolatedHome();
    await initialize(home);
    ...
    const changedProject = statusJson.projects.find(
      (project) => project.repositoryExclusions.length > 0,
    )!;
    expect(changedProject.repositoryExclusions).toHaveLength(1);
    const change = changedProject.repositoryExclusions[0]!;
    expect(change.target).toBe(`${realpathSync(join(repository, ".git", "info"))}/exclude`);
    expect(change.next.length).toBe(2);
    expect(change.current.length).toBe(0);
    ...
    for (const entry of change.next) {
  ...
  test("retiring the sole contributor of a missing exclusion file publishes nothing and retires the receipt", async () => {
    ...
    expect(statusVerbose.stdout).not.toContain("Git exclusions:\n-");
    const statusJson = JSON.parse((await runCli(home, "status", "--json")).stdout) as {
      globalBlockers: readonly unknown[];
      projects: readonly {
        state: { kind: string };
    ...
    // The exclusion target vanished with the deleted Project root: nothing to
    // publish or clean, and no Blocker for the unprovable topology.
    expect(retiringProject.repositoryExclusions).toHaveLength(0);
  ...
  test("a malformed exclusion section is skipped with one warning during intentional-deletion retirement", async () => {
    const home = isolatedHome();
    await initialize(home);
    ...
    expectExitCode(status, 0);
    const statusPayload = JSON.parse(status.stdout) as {
      projects: readonly { warnings: readonly { message: string }[] }[];
    };
    expect([...new Set(statusPayload.projects.flatMap((project) =>
      project.warnings.map((warning) => warning.message)
    ))].filter((message) => message.includes("malformed or duplicate"))).toHaveLength(1);
    const applied = await runCli(home, "update", "--json");
    ...
    expect([...(applyPayload.warnings ?? []), ...applyPayload.projects.flatMap((project) =>
      project.warnings.map((warning) => warning.message)
    )].filter((message) => message.includes("malformed or duplicate"))).toHaveLength(1);
    ...
    // The retired nested receipt leaves with its removal; the root receipt stays.
    expect(state.receipts).toHaveLength(1);
  });
  ...
  test("keeps retirement of a missing exclusion file retryable when state publication fails", async () => {
    ...
    expectExitCode(failed, 1);
    expect(failed.stderr).toContain("Update failed");
    expect(existsSync(exclude)).toBe(false);
  ...
  test("an explicitly bound linked checkout gets its own Profile Installation", async () => {
    ...
    expectExitCode(status, 0);
    expect(humanText(status.stdout)).toContain(humanText(`${repository}: not installed yet`));
    expect(humanText(status.stdout)).toContain(humanText(`${worktree}: not installed yet`));
    expect(status.stdout).not.toContain("Selected setup:");

    const apply = await runCliWithPath(home, pathWithClaude, "update");
    ...
    expectExitCode(currentStatus, 0);
    expect(humanText(currentStatus.stdout)).toContain(humanText(`${repository}: up to date`));
    expect(humanText(currentStatus.stdout)).toContain(humanText(`${worktree}: up to date`));
    const state = parse(readFileSync(statePath(home), "utf8")) as {
  ...
  test("a symlinked Git info parent is skipped with one warning and does not block update", async () => {
    const home = isolatedHome();
    await initialize(home);
  ...
  test("an unprovable Git common directory warns once and never blocks installation", async () => {
    ...
    const result = await runCli(home, "update", "--json");
  ...
  test("a failed first Git update restores an originally absent exclusion file", async () => {
    const home = isolatedHome();
    await initialize(home);
  ...
  test("a failed first Git update removes the exclusion parent it safely created", async () => {
    const home = isolatedHome();
    await initialize(home);
  ...
  test("modified Git exclusion ownership is rewritten by update and removed by uninstall", async () => {
    const home = isolatedHome();
    await initialize(home);
    ...
    const applied = await runCli(home, "update");
    expectExitCode(applied, 0);
    expect(readFileSync(exclude).toString("utf8")).toContain("/.codex/hooks.json");
    expect(readFileSync(exclude).toString("utf8")).not.toContain("/unexpected");
  ...
  test("status summarizes a pending Git exclusion repair before update", async () => {
    const home = isolatedHome();
    await initialize(home);
    ...
    for (const command of ["status"]) {
      const result = await runCli(home, command);
      expectExitCode(result, 0);
      expect(result.stdout).toContain("Ready to update");
      expect(result.stdout).toContain("Details: apkit status --verbose");
    ...
    writeFileSync(exclude, "# unrelated local exclusion\n");
    const verboseRepaired = await runCli(home, "update", "--verbose");
  ...
  test("a later Git project failure leaves exclusion bytes untouched for the retry", async () => {
    const home = isolatedHome();
    await initialize(home);
    const repositories = [
    ...
    expectExitCode(failed, 1);
    // Exclusion publication runs after all Project writes; a failed run leaves
    // every target untouched (the cache self-heals on the retry).
    expect(readFileSync(firstExclude, "utf8")).not.toContain("# BEGIN Agent Profile Kit generated paths");
    expect(readFileSync(secondExclude).equals(secondBefore)).toBe(true);

    const converged = await runCli(home, "update");
    expectExitCode(converged, 0);
    for (const exclude of [firstExclude, secondExclude]) {
      expect(readFileSync(exclude, "utf8")).toContain("# BEGIN Agent Profile Kit generated paths");
    }
  });

  test("a blocked stale removal keeps its derived entries while the surviving Project retires", async () => {
    const home = isolatedHome();
    await initialize(home);
    ...
    // Exclusion entries are derived, not ownership evidence: the retiring root
    // Project's entries leave with its receipt while the blocked nested
    // Project's entries stay published for its surviving installation.
    const after = readFileSync(exclude).toString("utf8");
    expect(after).not.toContain("\n/.codex/hooks.json");
    expect(after).not.toContain("\n/.agent-profile-kit/codex/context.md");
    expect(after).toContain("/nested/.codex/hooks.json");
    expect(after).toContain("/nested/.agent-profile-kit/codex/context.md");
    expect(existsSync(join(repository, ".agent-profile-kit", "codex", "context.md"))).toBe(false);
    expect(existsSync(join(repository, "nested", ".codex", "hooks.json"))).toBe(true);
  });
  ...
  test("status ignores an unbound worktree created after update", async () => {
    const home = isolatedHome();
    await initialize(home);
    ...
    expectExitCode(status, 0);
    expect(humanText(status.stdout)).toContain(humanText(`${repository}: up to date`));
  ...
  test("status distinguishes current, stale source, drifted output, and absent owned output", async () => {
    ...
    expect(humanText(current.stdout)).toContain(humanText(`${projectPath}: up to date`));
    ...
    expect(humanText(drifted.stdout)).toContain(
      humanText(`${projectPath}: generated files changed`),
    );
    ...
    expect(humanText(absent.stdout)).toContain(
      humanText(`${projectPath}: generated files missing`),
    );
  });

  test("status reports output permission drift and update refreshes it", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = project();
    writeContextProfile(home);
    ...
    expectExitCode(status, 0);
    expect(humanText(status.stdout)).toContain(humanText(`${projectPath}: generated files changed`));
    expectExitCode(applied, 0);
  ...
  test("update refreshes drifted generated output without manual recovery", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = project();
    writeContextProfile(home);
    ...
    expectExitCode(driftStatus, 0);
    expect(humanText(driftStatus.stdout)).toContain(humanText(`${projectPath}: generated files changed`));
    expectExitCode(repaired, 0);
  ...
  test("a partial update failure retains complete committed-operation evidence distinct from pending state", async () => {
    const home = isolatedHome();
    await initialize(home);
    removeScaffoldedExample(home);
    writeContextProfile(home);
  ...
  test("status attributes blockers by canonical project identity instead of path prefix", async () => {
    ...
    expectExitCode(result, 2);
    expect(humanText(result.stdout)).toContain(
      humanText(`${first}: not installed yet`),
    );
    expect(humanText(result.stdout)).not.toContain(humanText(`${first}: needs attention`));
    expect(humanText(result.stdout)).toContain(humanText(`${second}: needs attention (blocked)`));
  });
  ...
  test("a reinstalled Project is installed again after uninstall", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = project();
    writeContextProfile(home);
    ...
    expectExitCode(status, 0);
    expect(status.stdout).not.toContain("not installed yet");
    expect(status.stdout).not.toContain("intended teardown");
  });

  test("update recreates generated output after the user deletes it by hand", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = project();
    writeContextProfile(home);
  ...
  test("update removes a no-longer-bound installation only when ownership is proven", async () => {
    const home = isolatedHome();
    await initialize(home);
  ...
  test("restoring an intentionally deleted root requires a new binding and Installation ID", async () => {
    ...
    // The follow-up update is a no-op around the same receipt.
    const applied = await runCli(home, "update");
    expectExitCode(applied, 0);
    const stillCurrent = parse(readFileSync(statePath(home), "utf8")) as {
      receipts: readonly { installation_id: string }[];
    };
    expect(stillCurrent.receipts[0]?.installation_id).toBe(restored.receipts[0]?.installation_id);
  });

  test("update removes a no-longer-desired Adapter output whose recorded hash still proves ownership", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = project();
    writeContextProfile(home);
  ...
  test("update restores a wholly absent owned file from current Workspace source without changing installation identity", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = project();
    writeContextProfile(home);
    ...
    for (const command of ["status"] as const) {
      ...
      expectExitCode(result, 0);
      expect(humanText(result.stdout)).toContain(humanText(`${projectPath}: generated files missing`));
  ...
  test("verbose keeps the digest-only source change when Profile list order meets drift", async () => {
    ...
    expect(humanText(status.stdout)).toContain(humanText(
      `${projectPath}: generated files changed (.agents/skills/base-skill) (source changed)`,
    ));
    expect(humanText(status.stdout)).toContain(
  ...
  test("update restores a wholly absent owned Skill directory with current Workspace bytes and modes", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = project();
    writeContextProfile(home);
  ...
  test("a copied Project directory installs cleanly at its new binding while the original is removed", async () => {
    ...
    // No receipt claims the copy, but its byte-identical generated output is
    // adopted at the new binding; the original Project is no longer bound, so
    // its proven output is removed.
    expectExitCode(result, 0);
    expect(result.stderr).toBe("");
  ...
  test("update --all writes no Project when Installation State is globally unreadable", async () => {
    const home = isolatedHome();
    await initialize(home);
  ...
  test("update --all commits healthy Projects around a Project-scoped destination blocker", async () => {
    const home = isolatedHome();
    await initialize(home);
  ...
  test("update --blocked performs no writes when every selected Project is Blocked (#455)", async () => {
    const home = isolatedHome();
    await initialize(home);
    const healthy = project("agent-profile-kit-filtered-apply-healthy-");
    const blocked = project("agent-profile-kit-filtered-apply-blocked-");
    writeContextProfile(home);
    writeFileSync(
      configPath(home),
      `schema_version: 2\nworkspace: ${workspacePath(home)}\nbindings:\n` +
    ...
    const result = await runCli(home, "update", "--all", "--blocked");

    expectExitCode(result, 2);
    expect(result.stderr).toBe("");
    expect(result.stdout).toStartWith("✖ Update blocked");
    expect(result.stdout).toContain("Blocker:");
    // The unselected never-installed Project is not installed either.
    expect(existsSync(join(blocked, ".codex", "hooks.json")) &&
      readFileSync(join(blocked, ".codex", "hooks.json"), "utf8")).toBe("project-owned\n");
    expect(existsSync(join(healthy, ".agent-profile-kit"))).toBe(false);
    expect(existsSync(join(healthy, ".codex"))).toBe(false);
    expect(existsSync(statePath(home))).toBe(false);
  });

  test("update --blocked with no Blocked Projects changes nothing and exits clean (#455)", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = gitRepository("agent-profile-kit-filtered-apply-clean-");
    writeContextProfile(home);
    bind(home, projectPath);
    expectExitCode(await runCli(home, "update"), 0);
    const contextFile = join(projectPath, ".agent-profile-kit", "codex", "context.md");
    ...
    expectExitCode(result, 0);
    // No Project is selected, so the stale Project keeps its installed bytes.
    expect(readFileSync(contextFile, "utf8")).toBe(before);
    expect(existsSync(statePath(home))).toBe(true);
  });

  test("update --all --stale commits only selected stale Projects and leaves never-installed Projects untouched (#455)", async () => {
    const home = isolatedHome();
    await initialize(home);
    removeScaffoldedExample(home);
    writeContextProfile(home);
    const stale = project("agent-profile-kit-filtered-stale-committed-");
    const never = project("agent-profile-kit-filtered-stale-never-");
    writeFileSync(
      configPath(home),
      `schema_version: 2\nworkspace: ${workspacePath(home)}\nbindings:\n` +
        `  - project: ${stale}\n    profile: coding\n    hosts: [codex]\n`,
    ...
    // Both Projects are pending in the unfiltered fleet: the installed one
    ...
    // The selected committed write remains visible in the receipt (#451).
    expect(
      readFileSync(join(stale, ".agent-profile-kit", "codex", "context.md"), "utf8"),
    ).toContain("Shared source change.");
    // The machine payload carries the same write scope.
    const machine = await runCli(home, "status", "--all", "--json");
    expectExitCode(machine, 0);
    const payload = JSON.parse(machine.stdout) as {
      readonly projects: readonly {
        readonly canonicalProject: string;
    ...
    const states = new Map(payload.projects.map((entry) => [entry.canonicalProject, entry.state.kind]));
    expect(states.get(realpathSync(stale))).toBe("current");
    expect(states.get(realpathSync(never))).toBe("addition");
  });

  test("filtered update leaves an unselected Project's Git exclusion section byte-identical (#455)", async () => {
    const home = isolatedHome();
    await initialize(home);
    removeScaffoldedExample(home);
    writeContextProfile(home);
    const stale = gitRepository("agent-profile-kit-filtered-exclude-stale-");
    const unselected = gitRepository("agent-profile-kit-filtered-exclude-other-");
    // A second Profile with its own Context keeps the unselected Project
    ...
    expectExitCode(await runCli(home, "update", "--all"), 0);
    // Both Projects hold installed receipts contributing exclusion targets,
    ...
    const excludePath = join(unselected, ".git", "info", "exclude");
    const gutted = readFileSync(excludePath, "utf8")
      .split("# BEGIN Agent Profile Kit generated paths")[0];
    writeFileSync(excludePath, gutted!);
    const unselectedBefore = readFileSync(excludePath, "utf8");
    const staleExclude = join(stale, ".git", "info", "exclude");
    const staleBefore = readFileSync(staleExclude, "utf8");

    const result = await runCli(home, "update", "--all", "--stale");

    expectExitCode(result, 0);
    expect(readFileSync(excludePath, "utf8")).toBe(unselectedBefore);
    expect(readFileSync(staleExclude, "utf8")).toBe(staleBefore);
  });
  ...
  test("update --all commits a healthy Project around another Project's modified Git exclusion", async () => {
    const home = isolatedHome();
    await initialize(home);
    ...
    // A modified exclusion section no longer blocks anything: both Projects
    // converge from stale source and the derived exclusion rewrite.
    expectExitCode(result, 0);
    expect(readFileSync(healthyContext, "utf8")).toContain("Updated healthy Project.");
    expect(readFileSync(blockedExclude, "utf8")).not.toContain("/unexpected");
    expect(readFileSync(blockedExclude, "utf8")).toContain("/.codex/hooks.json");
    expect(readFileSync(blockedContext, "utf8")).toContain("Updated healthy Project.");
    expect(blockedContext).toBeDefined();
    void blockedBefore;
  });
  ...
  test("update --all JSON separates committed work from fresh blocked and current Project results", async () => {
    const home = isolatedHome();
    await initialize(home);
    ...
    const result = await runCli(home, "update", "--all", "--json");
  ...
  test("a moved Git project converges when its destination shares another repository exclusion contribution", async () => {
    ...
    const clean = await runCli(home, "update");
  ...
  test("Profiles selecting Skills install portable packages into Codex project discovery", async () => {
    ...
    const deselect = await runCli(home, "update");
  ...
  test("packed CLI preserves model-invocation policy across Codex and Claude Hosts", async () => {
    ...
    expectExitCode(await runCli(conflictHome, "update"), 0);
    ...
    const conflictApply = await runCli(conflictHome, "update");
    ...
    expectExitCode(status, 0);
    const apply = await runCliWithPath(home, pathValue, "update");
    expectExitCode(apply, 0);
  ...
  test("packed CLI Skills-only Profile validates, applies, and uninstalls without Context machinery", async () => {
    ...
    writeFileSync(configPath(home), `schema_version: 2\nworkspace: ${workspacePath(home)}\nbindings: []\n`);
    const uninstall = await runCliWithPath(home, pathValue, "update");
    expectExitCode(uninstall, 0);
    expect(existsSync(join(projectPath, ".agents", "skills", "review-pr"))).toBe(false);
  ...
  test("tracked exact planned Codex Skill destinations block preflight", async () => {
    ...
    const result = await runCli(home, "update");
    expectExitCode(result, 2);
    expect(result.stdout).toContain("Update blocked");
  ...
  test("packed CLI Claude-only status → update → status → uninstall installs unscoped Context", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = project();
    ...
    const status = await runCliWithPath(home, pathWithClaude, "status", "--verbose");
    expectExitCode(status, 0);
    expect(humanText(status.stdout)).toContain(humanText(`${projectPath}: not installed yet`));
  ...
  test("packed CLI status stays probe-free for Claude and update writes material when the CLI is too old", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = project();
    writeContextProfile(home);
  ...
  test("packed CLI Antigravity Context supports mixed lifecycle operations without touching project instructions", async () => {
    ...
    const apply = await runCliWithPath(home, pathWithHosts, "update");
    expectExitCode(apply, 0);
    expect(apply.stdout).toContain("First use:");
    expect(humanText(apply.stdout)).toContain(
    ...
    const deselection = await runCliWithPath(home, pathWithHosts, "update");
  ...
  test("packed CLI status stays probe-free for Antigravity and update writes material for old agy evidence", async () => {
    const home = isolatedHome();
    await initialize(home);
  ...
  test("packed CLI Grok-only status → update → status → uninstall installs unscoped Context", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = project();
    ...
    const apply = await runCliWithPath(home, pathWithGrok, "update");
    expectExitCode(apply, 0);
  ...
  test("packed CLI status stays probe-free for Grok, update warns for missing CLI, and installs Skills when ready", async () => {
    const home = isolatedHome();
    await initialize(home);
    const projectPath = project();
    writeContextProfile(home);
  ...
  test("packed fleet status executes no Agent Host process and update probes each unique requirement once", async () => {
    const home = isolatedHome();
    await initialize(home);
    removeScaffoldedExample(home);
    writeContextProfile(home);
    ...
    const apply = await runCliWithPath(home, pathWithHosts, "update", "--json");
    expectExitCode(apply, 0);
    const counts = {
      claude: 0,
      codex: 0,
  ...
  test("legacy plan and run interfaces are removed", async () => {
    const home = isolatedHome();
  ...
  test("retired apply points at update without a compatibility execution path", async () => {
    const home = isolatedHome();
...
describe("agent-profile-kit install (selection and output in one action)", () => {
  test("install records the canonical cwd and installs output when no project argument is supplied", async () => {
    const home = isolatedHome();
    await initialize(home);
    writeContextProfile(home);
  ...
  test("install inside bound Project renders recognizable identity across created, unchanged, and replaced", async () => {
    const home = isolatedHome();
    await initialize(home);
    ...
    const bindCreated = await runCliAt(
      home,
      projectPath,
      "install",
      "coding",
    ...
    const unchangedRoot = await runCliAt(
      home,
      projectPath,
      "install",
      "coding",
    ...
    const unchangedSub = await runCliAt(
      home,
      subDir,
      "install",
      "coding",
  ...
  test("identical install is idempotent and does not rewrite Local Configuration", async () => {
    const home = isolatedHome();
    await initialize(home);
    writeContextProfile(home);
    const projectPath = project();
  ...
  test("install over an existing binding replaces it in the same action without a --replace flag", async () => {
    const home = isolatedHome();
    await initialize(home);
    writeContextProfile(home, "coding");
    writeContextProfile(home, "ops");
  ...
  test("install restates Profile and Hosts of the existing installation with an old → new receipt", async () => {
    const home = isolatedHome();
    await initialize(home);
    writeContextProfile(home);
    writeContextProfile(home, "ops");
  ...
  test("install with changed Hosts installs the new output in the same action", async () => {
    const home = isolatedHome();
    await initialize(home);
    writeContextProfile(home);
    const projectPath = project();
    ...
    const replaced = await runCliWithPath(
      home,
      pathWithClaude,
      "install",
      "coding",
  ...
  test("install with a smaller Host set removes dropped-Host output in the same action", async () => {
    const home = isolatedHome();
    await initialize(home);
    writeContextProfile(home);
    const projectPath = project();
    ...
    expectExitCode(await runCliWithPath(home, pathWithClaude, "update"), 0);
  ...
  test("successful install preserves unrelated configuration, comments, and bindings", async () => {
    const home = isolatedHome();
    await initialize(home);
    writeContextProfile(home);
  ...
  test("install turns an empty Workspace missing-Profile error into an authoring next step", async () => {
    const home = isolatedHome();
    await initialize(home);
    removeScaffoldedExample(home);
    const projectPath = project();
  ...
  test("install writes only its Project output and leaves Host configuration and the Workspace alone", async () => {
    const home = isolatedHome();
    await initialize(home);
    writeContextProfile(home);
    const projectPath = project();
  ...
  test("install preserves CRLF line endings in Local Configuration", async () => {
    const home = isolatedHome();
    await initialize(home);
    writeContextProfile(home);
    const projectPath = project();
  ...
  test("install preserves a hardened Local Configuration file mode", async () => {
    const home = isolatedHome();
    await initialize(home);
    writeContextProfile(home);
    const projectPath = project();
  ...
  test("the retired bind command points at install", async () => {
    const home = isolatedHome();
...
describe("responsive lifecycle reports", () => {
  test("packed lifecycle output follows TTY width while pipes and JSON stay deterministic", async () => {
    ...
    const applied = await runCliInPty(home, 40, "update");
    ...
    expectExitCode(blocked, 2);
    expect(blocked.stdout).toContain("Cannot update");
  ...
  test("colored focused guides carry only authored categories on wrapped lines", async () => {
    ...
    expect(colored.stdout.split("\n").some((line) => line.includes("\u001b[1m"))).toBe(true);
  });
});
...
describe("apkit root help", () => {
  ...
  test("-v does not select verbose output or introduce a short alias for --verbose", async () => {
    ...
    const applyShort = await runCli(home, "update", "-v");
    expectExitCode(applyShort, 1);
    expect(applyShort.stderr).toContain("update does not accept argument '-v'");

    const applyUpperV = await runCli(home, "update", "-V");
    expectExitCode(applyUpperV, 1);
    expect(applyUpperV.stderr).toContain("update does not accept argument '-V'");
  });
  ...
  test("root help leads with the four-step first run", async () => {
    ...
    const bindIndex = result.stdout.indexOf("apkit install", initIndex + 1);
    const statusIndex = result.stdout.indexOf("apkit status", bindIndex + 1);
    const applyIndex = result.stdout.indexOf("apkit update", statusIndex + 1);
  ...
  test("non-help trailing arguments do not self-suggest a known command", async () => {
    const home = isolatedHome();
    const result = await runCli(home, HELP_COMMAND, "status", "unexpected");
  ...
  test("focused install help documents the one-action selection and output", async () => {
    ...
    for (const result of [...aliasHelp]) {
      expectExitCode(result, 0);
      expect(result.stdout).toContain("--auto-confirm");
      expect(result.stdout).toContain("--replace-changed");
    }
  });

  test("retired bind help points at install", async () => {
    const home = isolatedHome();
    for (const arguments_ of [["help", "bind"], ["bind", "--help"]] as const) {
      const result = await runCli(home, ...arguments_);
      expectExitCode(result, 1);
      expect(result.stderr).toContain("bind was replaced by install");
    }
  ...
  test("focused binding help names Hosts from each command's supported capability set", async () => {
    const home = isolatedHome();
    const bindHelp = await runCli(home, "help", "install");
  ...
  test("interactive root help adds the compact identity and semantic color while pipes stay plain", async () => {
    ...
    for (const heading of ["First run:", "Common commands:", "More commands:"]) {
      expect(interactive.stdout).toContain(`\u001b[1m${heading}\u001b[0m`);
    }
  ...
  test("representative invalid arguments exit nonzero, explain the error, and show the relevant command usage", async () => {
    ...
    const missingProfile = await runCli(home, "install");
    expectExitCode(missingProfile, 1);
    expect(missingProfile.stderr).toContain("install requires a Profile name");
    expect(missingProfile.stderr).toContain("Usage: apkit install <profile>");
    ...
    expect(tooManyInitPaths.stderr).not.toContain("Usage: apkit install");
...
describe("apkit list", () => {
  ...
  test("help distinguishes Project inventory from lifecycle diagnostics", async () => {
    ...
    expect(statusHelp.stdout).toContain("complete read-only update plan");
...
describe("apkit temporary Profile installation (Codex)", () => {
  ...
  test("install-temp / remove-temp complete Codex lifecycle with a versioned receipt and isolation", async () => {
    ...
    bind(home, boundProject, "coding");
    const applyBound = await runCli(home, "update");
    ...
    // Exclusion entries are derived at write time and no longer recorded in
    // the receipt or Installation State.
  ...
  test("install-temp rejects unknown Profile, unsupported Host, missing Project, and tracked destinations before writes", async () => {
    ...
    expect(unknownProfilePayload.schemaVersion).toBe(9);
    expect(existsSync(join(projectPath, ".agent-profile-kit"))).toBe(false);
  ...
  test("install-temp does not create Project Bindings or invoke global update side effects", async () => {
    const home = isolatedHome();
    await initialize(home);
    removeScaffoldedExample(home);
    writeContextProfile(home, "coding");
  ...
  test("remove-temp blocks a drifted temporary output that Git now tracks", async () => {
    ...
    // The tracked output, its index entry, and the active temporary receipt
    // all survive the blocked removal.
    expect(existsSync(tracked)).toBe(true);
...
describe("apkit temporary Profile installation (Claude Code parity)", () => {
  ...
  test("install-temp / remove-temp complete Claude lifecycle with versioned receipt and Claude Adapter outputs", async () => {
    ...
    const applyBound = await runCliWithClaude(home, "update");
    expectExitCode(applyBound, 0);
    const configBefore = readFileSync(configPath(home));
  ...
  test("ordinary and temporary Codex installations use the same shared Skill package shape", async () => {
    ...
    const applyDisabled = await runCliWithClaude(home, "update");
    ...
    expectExitCode(await runCliWithClaude(home, "update"), 0);
...
describe("apkit temporary Profile installation (OpenCode parity)", () => {
  ...
  test("install-temp / remove-temp complete OpenCode lifecycle with versioned receipt and OpenCode Adapter outputs", async () => {
    ...
    const applyBound = await runCliWithOpenCode(home, "update");
    expectExitCode(applyBound, 0);
    const configBefore = readFileSync(configPath(home));
    const boundContextBefore = readFileSync(
...
describe("repository exclusion contribution is best-effort bookkeeping (#379)", () => {
  const BEGIN = "# BEGIN Agent Profile Kit generated paths";
  const END = "# END Agent Profile Kit generated paths";

  function excludePath(repository: string): string {
    return join(repository, ".git", "info", "exclude");
  }

  function ownedSectionEntries(source: string): readonly string[] {
    const begin = source.indexOf(BEGIN);
    const end = source.indexOf(END);
    if (begin === -1 || end === -1) return [];
    return source.slice(begin + BEGIN.length + 1, end).split("\n").filter(Boolean);
  }

  /** Install one coding Profile into one Git repository and return its derived exclude entries. */
  async function installIntoGitRepository(prefix = "agent-profile-kit-exclusion-"): Promise<{
    ...
    const home = isolatedHome();
    await initialize(home);
    const repository = gitRepository(prefix);
    const exclude = excludePath(repository);
    writeFileSync(exclude, "# repository-local author bytes\n*.scratch\n");
    writeContextProfile(home);
    bind(home, repository);
    const apply = await runCli(home, "update", "--json");
    expectExitCode(apply, 0);
    const derived = ownedSectionEntries(readFileSync(exclude, "utf8"));
    expect(derived.length).toBeGreaterThan(0);
    return { home, repository, exclude, derived };
  }

  async function statusPayload(home: string): Promise<{
  ...
  }> {
    {
      ...
      expectExitCode(result, 0);
      const payload = JSON.parse(result.stdout) as {
        readonly projects: readonly { readonly warnings: readonly { readonly message: string }[] }[];
      };
      return {
        exitCode: result.exitCode,
        stdout: result.stdout,
        warnings: payload.projects.flatMap((project) =>
          project.warnings.map((warning) => warning.message)
        ),
      };
    }
  ...
  test("an emptied exclude file leaves status and update at exit 0 and rewrites the owned section", async () => {
    const { home, repository, exclude, derived } = await installIntoGitRepository(
      "agent-profile-kit-emptied-exclude-",
    );
    writeFileSync(exclude, "");

    const status = await statusPayload(home);
    expect(status.exitCode).toBe(0);
    expect(status.warnings.filter((warning) => warning.includes(excludePath(repository)))).toHaveLength(1);

    const unrelated = "# author bytes survive\n*.scratch\n";
    writeFileSync(exclude, unrelated);
    const apply = await runCli(home, "update");
    expectExitCode(apply, 0);

    const rewritten = readFileSync(exclude, "utf8");
    expect(ownedSectionEntries(rewritten)).toEqual(derived);
    expect(rewritten.startsWith(unrelated)).toBe(true);
    expect(existsSync(join(repository, ".codex", "hooks.json"))).toBe(true);
  });

  test("a modified owned section is rewritten from the current installation", async () => {
    const { home, repository, exclude, derived } = await installIntoGitRepository(
      "agent-profile-kit-modified-section-",
    );
    const prefix = "# repository-local author bytes\n*.scratch\n";
    writeFileSync(exclude, `${prefix}${BEGIN}\n/user-edited-entry\n${END}\n`);

    const status = await statusPayload(home);
    expect(status.exitCode).toBe(0);
    expect(status.warnings.some((warning) => warning.includes(exclude))).toBe(true);
    ...
    expect(rewritten).not.toContain("/user-edited-entry");
  });

  test("an absent section warns on status and is created by update", async () => {
    const { home, repository, exclude, derived } = await installIntoGitRepository(
      "agent-profile-kit-absent-section-",
    );
    const unrelated = "# repository-local author bytes\n*.scratch\n";
    writeFileSync(exclude, unrelated);
    ...
    const status = await statusPayload(home);
    expect(status.exitCode).toBe(0);
    expect(status.warnings.filter((warning) => warning.includes(exclude))).toHaveLength(1);
  ...
  test("an unwritable Git info parent produces one warning and does not block update", async () => {
    const { home, repository, exclude, derived } = await installIntoGitRepository(
      "agent-profile-kit-unwritable-exclude-",
    );
    // Corrupt the section so apply has a delta to publish, then make the
    // containing directory unwritable so the publication fails.
    writeFileSync(exclude, readFileSync(exclude, "utf8").replace(derived[0]!, "/unexpected"));
    ...
    expect([...new Set(applyPayload.projects.flatMap((project) =>
      project.warnings.map((warning) => warning.message)
    ))].filter((message) => message.includes(exclude))).toHaveLength(1);
    expect(readFileSync(exclude, "utf8")).toBe(before);
    expect(existsSync(join(repository, ".codex", "hooks.json"))).toBe(true);

    chmodSync(join(repository, ".git", "info"), 0o755);
    expectExitCode(await runCli(home, "update"), 0);
    expect(ownedSectionEntries(readFileSync(exclude, "utf8"))).toEqual(derived);
  });

  test("an absent Git info parent is created by update without blocking", async () => {
    const { home, repository, derived } = await installIntoGitRepository(
      "agent-profile-kit-absent-parent-",
    );
    const exclude = excludePath(repository);
    rmSync(join(repository, ".git", "info"), { recursive: true });
  ...
  test("a Project that stops being a Git repository installs without exclusion bookkeeping", async () => {
    ...
    // Blocker (exit 2); exclusion bookkeeping now simply stops updating, and the
  ...
  test("a non-Git Project installs identically apart from exclusion entries", async () => {
    const home = isolatedHome();
    await initialize(home);
    const directory = project("agent-profile-kit-plain-directory-");
    writeContextProfile(home);
    bind(home, directory);
    ...
    const apply = await runCli(home, "update");
    expectExitCode(apply, 0);
    expect(existsSync(join(directory, ".codex", "hooks.json"))).toBe(true);
    expect(existsSync(join(directory, ".agent-profile-kit", "codex", "context.md"))).toBe(true);
    // No Git boundary: no exclusion target exists to touch anywhere.
  ...
  test("uninstall removes the exclusion entries without a later update", async () => {
    ...
    for (const entry of derived) expect(after).not.toContain(entry);
    expect(after).not.toContain(BEGIN);
  });

  test("an unwritable exclusion target does not stall uninstall teardown", async () => {
    const { home, repository, exclude } = await installIntoGitRepository(
      "agent-profile-kit-unwritable-teardown-",
    );
    chmodSync(join(repository, ".git", "info"), 0o555);
    ...
    // removes the leftover cache bytes — the file is the user's to edit.
    chmodSync(join(repository, ".git", "info"), 0o755);
    expect(readFileSync(exclude, "utf8")).toContain(BEGIN);
  });

  test("receipts no longer record exclusion fields and the schema publishes version 9", async () => {
    const { home } = await installIntoGitRepository("agent-profile-kit-schema-v9-");
    const state = JSON.parse(readFileSync(statePath(home), "utf8")) as {
      schema_version: number;
      receipts: readonly { repository_exclusion?: unknown }[];
    };
...
describe("packed CLI new profile", () => {
  ...
  test("new profile refuses duplicate selections, invalid names, occupied destinations, and unknown flags without writing", async () => {
    ...
    // An occupied destination holding existing material is refused without
    // overwriting. The occupant's ID is its file name, so the refusal is the
    // duplicate-Artifact-ID diagnosis naming the existing file (US-015, #508).
    ...
    expectExitCode(occupied, 1);
    expect(occupied.stderr).toContain("A Profile named 'taken' already exists at profiles/taken.yaml");
    expect(occupied.stderr).toContain("Nothing was created or changed");
    // The structured diagnostic carries a runnable recovery command (INT-1).
...
describe("packed CLI changed-output replacement confirmation (#458, US-029-031, DEC-019)", () => {
  ...
  test("the answering flag replaces changed generated files without a prompt", async () => {
    ...
    const result = await runCli(home, "update", projectPath, "--replace-changed");
    expectExitCode(result, 0);
    expect(readFileSync(contextPath, "utf8")).toContain("Always preserve the project boundary.");
...
describe("packed CLI install missing-argument errors (#494, US-001, US-006)", () => {
  test("non-interactive install with missing arguments errors without waiting for input", async () => {
    const home = isolatedHome();
    await initialize(home);
    writeContextProfile(home);
    const projectPath = homeGitRepository(home, "install-noninteractive");

    // Piped stdin: missing choices stay missing (#495 owns the pickers), so the
    // delivered argument error proves the invocation never waited.
    const result = await runCliAt(home, projectPath, "install");
    expectExitCode(result, 1);
    expect(humanText(result.stderr)).toContain("install requires a Profile name");
    expect(humanText(result.stderr)).toContain("Usage: apkit install");
  ...
  test("fully specified packed install refuses without --auto-confirm on piped input", async () => {
    const home = isolatedHome();
    await initialize(home);
    writeContextProfile(home);
